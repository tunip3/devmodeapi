...
class XboxOneDevmodeApi(object):
    ...
    def _delete(self, endpoint, *args, **kwargs):
        return self.session.delete(self.base_url + endpoint, headers=self._csrf_header, *args, **kwargs)
    ...
    def launchapp(self, relativeappid):
        rai = b64encode(relativeappid.encode('utf8')).decode('utf8')
        rai = quote_plus(rai)
        url = "/api/taskmanager/app?appid=" + rai
        return self._post(url).status_code

    def setmachinename(self, name):
        name = b64encode(name.encode('utf8')).decode('utf8')
    ...
    def add_newaccount(self, email, password, autosignin):
        url = "/ext/user"
        json = {"Users": [
            {"AutoSignIn": autosignin, "EmailAddress": str(email), "Password": str(password), "SignedIn": True}]}
        return self._put(url, json=json).status_code

    def delete_account(self, email):
        url = "/ext/user"
        json = {"Users": [{"EmailAddress": str(email), "Delete": True}]}
        return self._put(url, json=json).status_code
...
if __name__ == '__main__':
    ...
    # for i in api.get_pidsfromprocessname('sshd.exe'):
    #    print(i)
